 ...
 */
export const ReviewArgsSchema = z
  .object({
    // Numeric validation
    confidence: z
      .string()
  ...
  .refine((data) => !(data.full && !data.files), {
    message: '--full requires --files to specify which files to review',
  })
  .refine((data) => !(data.full && data.base), {
    message:
      '--full and --base are incompatible. Use --full for whole-file review without git, or --base for git diff comparison.',
  });
