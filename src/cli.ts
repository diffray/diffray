...
async function runReview(verbose = false) {
  ...
  const result = await pipeline.execute(filteredDiffs, verbose);
  ...
  if (result.context.issues.length > 0) {
    const errorCount = result.context.issues.filter((i) => i.severity === "error").length;
    const warningCount = result.context.issues.filter((i) => i.severity === "warning").length;
    const infoCount = result.context.issues.filter((i) => i.severity === "info").length;
    const suggestionCount = result.context.issues.filter((i) => i.severity === "suggestion").length;
    ...
    log.chart(`${result.context.issues.length} issue(s): ${parts.join(", ")}`);

    // Display all issues
    log.newline();
    console.log(formatIssuesByFile(result.context.issues));
  }
